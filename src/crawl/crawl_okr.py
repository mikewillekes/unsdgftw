...
CORPUS_ID = 'OKR'
ORGANIZATION = 'World Bank Group Open Knowledge Repository'
...
def scrape_url(url, head):
    ...
    headers = ' '.join([normalize_whitespace(h.get_text()) for h in soup.select('h2.ds-div-head')])
    paragraphs = (' '.join([normalize_whitespace(p.get_text()) for p in soup.find('div', {'class': 'abstract'}).contents])).strip()
    
    soup.find('div', {'class': 'abstract'}).contents
    
    article_date = None
    fields = [normalize_whitespace(f.get_text()).strip() for f in soup.select('div.okr-item-page-field-wrapper')]
    if fields:
        for field in fields:
            if field.startswith('Published'):
                publish_date = field.split('Published ')[-1]
                article_date = dateparser.parse(publish_date)


    links = soup.select('a.bitstream-link')
    if links:
        for link in links:
            if link.get_text().strip() == 'English PDF':
                
                # World Bank uses relative links for download urls
                href = 'https://openknowledge.worldbank.org' + link['href'].lower()
